...
CompactFVector3 Triangle::GetCentroid() const {
	return { (verts[0].pos.x + verts[1].pos.x + verts[2].pos.x) / 3,
			 (verts[0].pos.y + verts[1].pos.y + verts[2].pos.y) / 3,
			 (verts[0].pos.z + verts[1].pos.z + verts[2].pos.z) / 3 };
}
